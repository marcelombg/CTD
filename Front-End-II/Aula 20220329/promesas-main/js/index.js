...
consultandoBaseDeDados
    .then((resposta) => {
        const adicionandoDados = document.querySelector('.tarjeta');
        let recebendoDados = baseDeDados.resultado[0];

        adicionandoDados.innerHTML = `
        <h4>${recebendoDados.nome.primeiro}</h4>
        <h4>${recebendoDados.email}</h4>
        <img src="${recebendoDados.imagem.grande}">
        `;

        console.log(baseDeDados.resultado[0].nome.value);
    })
    .catch((err) => {
        console.log(err);
    });
